...
class DatasetProperties():
    def __init__(self, 
                 ...
                 save_train_test_splits:str = None,
                 load_train_test_splits:str = None,
        ...
        if train_test_split:
            if load_train_test_splits is not None:
                self.train_labels, self.test_labels = utils.load_splits(load_train_test_splits)
            else:
                self.train_test_split()
                if save_train_test_splits is not None:
                    utils.save_train_test_split(self.train_labels, self.test_labels, save_train_test_splits)
            self.train_nir_images = [img for img in self.nir_images if utils.label_in_set(img, self.train_labels)]
            self.train_vis_images = [img for img in self.vis_images if utils.label_in_set(img, self.train_labels)]
            self.test_nir_images = [img for img in self.nir_images if utils.label_in_set(img, self.test_labels)]
            self.test_vis_images = [img for img in self.vis_images if utils.label_in_set(img, self.test_labels)]
        ...
        if same_label_pairs is not None:
            ...
            # for img_name in self.nir_images:
                modality, label, sub_index = utils.read_CASIA(img_name)
                ...
                for candidate in self.train_nir_images:
                    candidate_modality, candidate_label, candidate_sub_index = utils.read_CASIA(candidate)
        ...
        if subindex_for_label is not None:
            ...
            for img_name in tqdm(self.train_nir_images, desc ="Creating dictionnary of sub_index list:"):
                modality, label, sub_index = utils.utils.read_CASIA(img_name) #here modality should be 1
                key = f"NIR_{label}"
    ...
    def train_test_split(self, test_size=0.25):
        unique_labels = set()
        for img_name in self.nir_images:
            _, label, _ = utils.read_CASIA(img_name)
...
class HDTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        _, label, _ = utils.read_CASIA(img1_name)
        ...
        if same_class:
            if only_nir:    
                img2_name = utils.random_element_in_list(self.dp.same_label_pair[img1_name][0])
                img2 = Image.open(os.path.join(self.dp.nir_folder, f'{img2_name}.bmp'))  
            else:
                img2_name = utils.random_element_in_list(self.dp.same_label_pair[img1_name][1])
                img2 = Image.open(os.path.join(self.dp.vis_folder, f'{img2_name}.jpg')) 
        else:
            
            #FIND A NEW LABEL
            img2_name = utils.random_element_in_list(self.dp.train_vis_images)
            _, label2, _ = utils.read_CASIA(img1_name)
            while (label2 == label):
                img2_name = utils.random_element_in_list(self.dp.train_nir_images)
                _, label2, _ = utils.read_CASIA(img2_name)
            
            #CHOOSE NIR OR VIS
            if only_nir:
                img2_name = utils.random_element_in_list(self.dp.all_sub_index_list[f'NIR_{label2}'])
                img2 = Image.open(os.path.join(self.dp.nir_folder, f'{img2_name}.bmp'))     
            else:
                img2_name = utils.random_element_in_list(self.dp.all_sub_index_list[f'VIS_{label2}'])
                img2 = Image.open(os.path.join(self.dp.vis_folder, f'{img2_name}.jpg'))     
